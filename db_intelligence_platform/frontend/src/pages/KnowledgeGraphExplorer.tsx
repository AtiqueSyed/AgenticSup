...
export default function KnowledgeGraphExplorer() {
  ...
  useEffect(() => {
    ...
    if (!showDatabases) {
      filteredNodes = filteredNodes.filter((n: any) => !n.data?.label?.startsWith("[Database]"));
    }
    if (!showTables) {
      filteredNodes = filteredNodes.filter((n: any) => !n.data?.label?.startsWith("[Table]"));
    }
    if (!showColumns) {
      filteredNodes = filteredNodes.filter((n: any) => !n.data?.label?.startsWith("[Column]"));
    }

    const validNodeIds = new Set(filteredNodes.map((n: any) => n.id));
    filteredEdges = filteredEdges.filter((e: any) => validNodeIds.has(e.source) && validNodeIds.has(e.target));
  ...
  useEffect(() => {
    if (rfInstance) {
      setTimeout(() => rfInstance.fitView({ duration: 800, padding: 0.2 }), 100);
    }
  }, [showDatabases, showTables, showColumns, rfInstance, rawGraphData]);
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center cursor-pointer bg-white p-2 px-3 rounded-lg border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors">
            <div className="relative">
              ...
              />
              <div className={`block w-10 h-6 rounded-full transition-colors ${showTables ? 'bg-indigo-500' : 'bg-slate-300'}`}></div>
              <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${showTables ? 'transform translate-x-4' : ''}`}></div>
            </div>
            <div className="ml-3 text-sm font-medium text-slate-700">
              Tables
            </div>
          ...
          <label className="flex items-center cursor-pointer bg-white p-2 px-3 rounded-lg border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors">
            <div className="relative">
              ...
              />
              <div className={`block w-10 h-6 rounded-full transition-colors ${showColumns ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
              <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${showColumns ? 'transform translate-x-4' : ''}`}></div>
            </div>
            <div className="ml-3 text-sm font-medium text-slate-700">
              Columns
            </div>
