...
class CreateNodeRequest(BaseModel):
    id: str
    name: str
    type: str # 'Database', 'Table', 'Column', 'Entity'
    description: Optional[str] = ""

class CreateEdgeRequest(BaseModel):
    source: str
    target: str
    type: str # e.g. CONTAINS, HAS_TABLE, HAS_COLUMN, MAPS_TO, RELATES_TO

@router.post("/graph/node")
async def create_custom_node(request: CreateNodeRequest):
    if not neo4j_driver:
        raise HTTPException(status_code=500, detail="Neo4j not connected")
    async with neo4j_driver.session() as session:
        label = request.type.capitalize()
        if label not in {"Database", "Table", "Column", "Entity"}:
            label = "Entity"
        query = f"MERGE (n:{label} {{id: $id}}) SET n.name = $name, n.label = $name, n.description = $desc RETURN n"
        await session.run(query, id=request.id, name=request.name, desc=request.description)
    return {"status": "created", "node_id": request.id}

@router.post("/graph/edge")
async def create_custom_edge(request: CreateEdgeRequest):
    if not neo4j_driver:
        raise HTTPException(status_code=500, detail="Neo4j not connected")
    rel_type = request.type.upper().replace("`", "")
    async with neo4j_driver.session() as session:
        query = f"""
            ...
            RETURN r
        """
        await session.run(query, source=request.source, target=request.target)
    return {"status": "created", "source": request.source, "target": request.target}
...
async def delete_custom_node(node_id: str):
    if not neo4j_driver:
        raise HTTPException(status_code=500, detail="Neo4j not connected")
    async with neo4j_driver.session() as session:
        await session.run("MATCH (n) WHERE n.id = $id DETACH DELETE n", id=node_id)
    return {"status": "deleted", "node_id": node_id}

@router.delete("/graph/edge/{source_id}/{target_id}/{edge_type}")
async def delete_custom_edge(source_id: str, target_id: str, edge_type: str):
    if not neo4j_driver:
        raise HTTPException(status_code=500, detail="Neo4j not connected")
    rel_type = edge_type.upper().replace("`", "")
    async with neo4j_driver.session() as session:
        query = f"""
            ...
            DELETE r
        """
        await session.run(query, src_id=source_id, tgt_id=target_id)
